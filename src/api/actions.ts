...
const API_URL = "https://musical-waddle-4jjxwjvgvqg62jj6x-3000.app.github.dev/api";

export const getPropertyData = async (Type: string): Promise<PropertyData> => {
  return new Promise<PropertyData>((resolve, reject) => {
    axios
      .get(`${API_URL}/car/${Type}`)
      .then((res) => {
        resolve({
          Type: res.data.make,
          Buying_Price: res.data.year,
          Rent_price: res.data.price,
